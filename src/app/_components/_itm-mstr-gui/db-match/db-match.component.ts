...
@Component({
    templateUrl: './db-match.component.html'
})
export class DBMatchComponent {

    componentTitle: string = 'DB Match';
    private successMsgs = [];
    private msgs = [];
    private reportUri = '/v1/data/item-master-data/api/search/item-match-extract/report?size=100'
    ...
    inputClick(item) {
        ...
        if (item && typeof item.inputFileName != 'undefined') {
            let linkUri = '/api/item-extract/report/link/' + item.itemMatchReportId;
            this._waitSpinnerService.showWait();
    ...
    _onRangeSubmit() {
        ...
        let uri = '/v1/data/item-master-data/item-match-report';
        this.isSubmit = true;
        this._waitSpinnerService.showWait();
    ...
    downloadOutput(item) {
        this.msgs = [];
        if (item && typeof item.itemMatchReportId != 'undefined') {
            let linkUri = '/api/item-extract/report/link/' + item.itemMatchReportId;
            this._waitSpinnerService.showWait();
            this._webServiceComm.httpGet(linkUri).subscribe(res => {
    ...
    cancel(item: Report) {
        ...
        this.confirmationService.confirm({
            message: 'Are you sure, you want to perform CANCEL for ' + 'REQ' + ("000000" + item.itemMatchReportId).slice(-6) + '?',
            accept: () => {
                let reqObj = new Report();
                reqObj = item;
                reqObj.status = "C";
    ...
    upload() {
        ...
        this._http.post(this._webServiceComm._resolveUrl(uri), formData, this._getUploadHeaderOptions())
            ...
            data => {
                ...
                uploadResponse.itemMatchReportId = data.itemMatchReportId;
    ...
    private fileUpload(retrievedObject) {
        ...
        let uploaddatauri = '/api/item-extract/report/file/uploadData'
        this._waitSpinnerService.showWait();
